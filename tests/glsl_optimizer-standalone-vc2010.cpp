...
#if defined(__S3E__)
// assume you are using GLES 2.0 ; change the following if you are using GLES > 2.0
#define GOT_MORE_THAN_GLSL_120 0
#include "dirent.h"
#include <IwGL.h>
...
int main (int argc, const char** argv)
{
	...
	bool ok = TestFile (ctx[api], type==0, inname, testFolder + "/" + inname, testFolder + "/" + outname, api<=1, false, false);
	if (!ok)
	{
		++errors;
	}
